...
const GOSSIP_LINES = [
  "Mahallede herkes aynı konuyu konuşuyor, kimse yüzüne söylemiyor ama...",
  "İş yerindeki o çift aslında çoktan ayrılmış, haberiniz yok 😶",
  "Bu şehrin en popüler mekanda dün gece olay çıktı, kulaklarım hâlâ çınlıyor.",
  "Üniversite kampüsünde dolaşan gıybet final haftasını unutturdu.",
  "Belediye binası civarında dolaşan söylenti resmen viral oldu.",
];
...
const COMMENT_LINES = [
  "Ben de duydum, detay lazım 👀",
  "Kimden bahsediyorsun tam olarak?",
  "Bu şehirde herkes birbirini tanıyor zaten.",
  "Yakında herkes bilir, çok gizli kalmaz.",
  "Az önce yan masadan aynısını duydum!",
];
...
let sql = `-- GıyBet: Türkiye 81 il örnek gıybet + yorum seed
...
const valueRows = CITIES.map(
  ([name, lat, lng]) => `      ('${esc(name)}', ${lat}, ${lng})`,
);
sql += valueRows.join(",\n");
sql += `
...
const outPath = join(__dirname, "..", "supabase", "seed_turkey_gossips.sql");
writeFileSync(outPath, sql, "utf8");
console.log(`Written ${CITIES.length} cities × 4 gossips → ${outPath}`);
